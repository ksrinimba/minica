...
func makeRootCert(key crypto.Signer, filename string) (*x509.Certificate, error) {
	...
	template := &x509.Certificate{
		Subject: pkix.Name{
			CommonName: "Srini Minica Root ca " + hex.EncodeToString(serial.Bytes()[:3]),
		},
		SerialNumber: serial,
		NotBefore:    time.Now(),
...
func sign(iss *issuer, domains []string, ipAddresses []string) (*x509.Certificate, error) {
	...
	roleID := asn1.ObjectIdentifier{1, 2, 840, 10070, 8, 1}
	...
	spinExt = append(spinExt, spinOid)
